...
public class SysAuthenticationServiceImpl implements ISysAuthenticationService {
    ...
    private void validateCaptcha(LoginInfo reqVO) {
        // 校验验证码
        ValidationUtil.validate(validator, reqVO, LoginInfo.CodeEnableGroup.class);
        CaptchaVO captchaVO = new CaptchaVO();
        captchaVO.setCaptchaVerification(reqVO.getCaptchaVerification());
        Result<Nullable> response = captchaService.verification(captchaVO);
        // 验证不通过
        if (response.getCode() != 0) {
            // 创建登录失败日志（验证码不正确)
            createLoginLog(null, reqVO.getUsername(), LoginResultEnum.CAPTCHA_CODE_ERROR);
    ...
    private void createLoginLog(Long userId, String username, LoginResultEnum loginResult) {
        ...
        reqDTO.setUserAgent(ServletUtil.getUserAgent());
    ...
    @Override
    public List<SysAccount> fetchOnlineAccounts() {
        ...
        for (Object principal : allPrincipals) {
            ...
            for (SessionInformation session : sessions) {
                ...
                String lastRequestDate = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(session.getLastRequest());
                log.info("sessionId: {}, lastRequestDate: {}", sessionId, lastRequestDate);
            }
        }
        return List.of();
    }
    ...
    @Override
    public Boolean kickAccount(String username) {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        List<SysMenu> menus = menuService.fetchMenusByUsername(username);
