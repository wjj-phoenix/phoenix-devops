...
import com.phoenix.devops.utils.ImageUtil;
...
public class DefaultCaptchaServiceImpl implements ISysCaptchaService {
    ...
    public CaptchaVO get() {
        // 原生图片
        BufferedImage originalImage = ImageUtil.fetchOriginalImage();
        ...
        // 抠图图片
        String slidingBlockImageBase64 = ImageUtil.fetchSlidingBlockImage();
        BufferedImage slidingBlockImage = ImageUtil.base64StrToImage(slidingBlockImageBase64);
    ...
    public CaptchaVO check(CaptchaVO captchaVO) {
        ...
        if (exists) {
            ...
            throw new IllegalArgumentException(RespEnum.API_CAPTCHA_COORDINATE_ERROR.getMessage());
        }
        // 校验成功，将信息存入缓存
    ...
    @Override
    public Result<Nullable> verification(CaptchaVO captchaVO) {
        if (captchaVO == null) {
            return Result.failure(RespEnum.NULL_ERROR.getMessage());
        }
    ...
    public CaptchaVO pictureTemplatesCut(BufferedImage originalImage, BufferedImage slidingBlockImage, String slidingBlockImageBase64) {
        ...
        if (captchaInterferenceOptions > 0) {
            ...
            if (originalWidth - x - 5 > slidingBlockWidth * 2) {
                // 在原扣图右边插入干扰图
                position = RandomUtil.randomInt(x + slidingBlockWidth + 5, originalWidth - slidingBlockWidth);
            } else {
                // 在原扣图左边插入干扰图
                position = RandomUtil.randomInt(100, x - slidingBlockWidth - 5);
            }
            while (true) {
                String s = ImageUtil.fetchSlidingBlockImage();
                if (!slidingBlockImageBase64.equals(s)) {
                    interferenceByTemplate(originalImage, Objects.requireNonNull(ImageUtil.base64StrToImage(s)), position);
                    break;
                }
            }
        }
        if (captchaInterferenceOptions > 1) {
            while (true) {
                String s = ImageUtil.fetchSlidingBlockImage();
                if (!slidingBlockImageBase64.equals(s)) {
                    Integer randomInt = RandomUtil.randomInt(slidingBlockWidth, 100 - slidingBlockWidth);
                    interferenceByTemplate(originalImage, Objects.requireNonNull(ImageUtil.base64StrToImage(s)), randomInt);
                    break;
                }
            }
        }
        ...
        try {
            ...
            fastMap.put(codeKey, JsonUtil.toJSONString(point));
